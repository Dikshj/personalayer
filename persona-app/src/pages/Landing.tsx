...
function FingerprintCanvas() {
  ...
  useEffect(() => {
    ...
    const spokes = 72;
    const dims = [
      { color: "#c97f62", speed: 0.0007, amp: 0.28 },
      { color: "#5f8466", speed: 0.0005, amp: 0.22 },
      { color: "#c97f62", speed: 0.0009, amp: 0.19 },
      { color: "#5f8466", speed: 0.0006, amp: 0.31 },
      { color: "#c97f62", speed: 0.0008, amp: 0.17 },
      { color: "#5f8466", speed: 0.0004, amp: 0.25 },
    ];
    ...
    const draw = (timestamp: number) => {
      ...
      for (let i = 0; i < 16; i += 1) {
        ...
        ctx.strokeStyle = "rgba(201, 127, 98,0.08)";
        ctx.lineWidth = 0.5;
        ctx.globalAlpha = 1;
        ctx.stroke();
      }
...
function PersonaDemo() {
  const rows = [
    ["Price sensitivity", "62%", "62%", "bg-[#c97f62]"],
    ["Feature curiosity", "88%", "88%", "bg-[#5f8466]"],
    ["Churn risk", "24%", "24%", "bg-[#b0573f]"],
  ];

  return (
    <div className="pl-persona-demo">
      <div className="mb-6 flex items-center gap-4">
        <div className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-gradient-to-br from-[#c97f62] to-[#5f8466] font-bold text-white">
          DJ
        </div>
        <div>
          <div className="font-display text-sm font-semibold text-[#1f1e1c]">Digital Persona #4,421</div>
          <div className="text-xs text-[#6e6d67]">Urban / 28-34 / Mid-income / Tech-adjacent</div>
        </div>
      </div>
      ...
      <div className="mb-5 text-xs text-[#6e6d67]">Purchase likelihood signals</div>
      <div className="space-y-3">
        {rows.map(([name, label, width, color]) => (
          <div key={name} className="grid grid-cols-[96px_1fr_34px] items-center gap-3 text-xs">
            <span className="text-[#1f1e1c]/80">{name}</span>
            <span className="h-1 overflow-hidden rounded-full bg-white/10">
              <span className={`block h-full rounded-full ${color}`} style={{ width }} />
            </span>
            <span className="font-mono text-[#6e6d67]">{label}</span>
          </div>
        ))}
      </div>
      <div className="mt-6 flex items-center justify-between rounded-xl border border-[#5f8466]/20 bg-[#5f8466]/10 px-4 py-3">
        <div>
          <div className="text-xs text-[#6e6d67]">Overall fit score for your product</div>
          <div className="mt-0.5 text-[11px] text-[#6e6d67]">Based on persona context + product brief</div>
        </div>
        <div className="font-display text-xl font-bold text-[#5f8466]">87%</div>
      </div>
    </div>
  );
}
...
export default function Landing() {
  ...
  return (
    <main className="pl-landing min-h-dvh bg-[#fbfbfa] text-[#1f1e1c]">
      <header className="fixed inset-x-0 top-0 z-50 border-b border-on-surface/[0.08] bg-[#fbfbfa]/85 px-5 py-4 backdrop-blur md:px-12">
        <div className="mx-auto flex max-w-6xl items-center justify-between">
          <Link to="/" className="font-display flex items-center gap-2 text-lg font-bold text-[#1f1e1c]">
            <img src="/personalayer-mark.svg" alt="" className="h-8 w-8" />
            PersonaLayer
          </Link>
          <nav className="hidden items-center gap-8 text-sm text-[#6e6d67] md:flex">
            <a href="#for-you" className="hover:text-[#1f1e1c]">For You</a>
            <a href="#for-business" className="hover:text-[#1f1e1c]">For Business</a>
            <a href="#how" className="hover:text-[#1f1e1c]">How It Works</a>
            <a href="#usecases" className="hover:text-[#1f1e1c]">Use Cases</a>
          </nav>
      ...
      <section className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden px-5 pb-16 pt-32 text-center">
        ...
        <h1 className="fade-up delay-1 font-display relative z-10 max-w-5xl text-[clamp(2.35rem,7vw,5.5rem)] font-bold leading-[1.05]">
          ...
          <br />
          <span className="text-[#c97f62]">Everything, personalized.</span>
          <br />
          <span className="text-[#5f8466]">Nothing, exploited.</span>
        </h1>
        <p className="fade-up delay-2 relative z-10 mt-7 max-w-2xl text-base leading-8 text-[#6e6d67] md:text-lg">
          ...
          <div className="pl-ticker">
            {tickerItems.map(([tag, text], i) => (
              <span key={`${tag}-${i}`} className="pl-ticker-item">
                ...
                {text}
                <span className="text-[#6e6d67]/50">/</span>
              </span>
            ))}
          </div>
        </div>
      ...
      <section className="border-y border-on-surface/[0.08] bg-[#f4f4f2] px-5 py-12">
        <div className="mx-auto grid max-w-6xl gap-8 text-center sm:grid-cols-2 lg:grid-cols-4">
          {STATS.map((stat, i) => (
            <Reveal key={stat.label} delay={i * 70}>
              <div>
                <div className="font-display mb-2 text-4xl font-bold tracking-tight text-[#1f1e1c]">
                  {stat.value}
                  {stat.unit && <span className="ml-1 text-base text-[#c97f62]">{stat.unit}</span>}
                </div>
                <div className="text-sm text-[#6e6d67]">{stat.label}</div>
              </div>
            </Reveal>
          ))}
        </div>
      ...
      <section id="for-you" className="px-5 py-24">
        <div className="mx-auto max-w-6xl">
          ...
          <p className="mt-4 max-w-2xl text-[#6e6d67]">
            One persona layer. Two different superpowers, depending on which side you are on.
          </p>

          <div className="mt-12 grid overflow-hidden rounded-3xl border border-on-surface/[0.08] bg-on-surface/[0.04] lg:grid-cols-2">
            <Reveal>
              <article className="pl-audience-panel border-b border-on-surface/[0.08] lg:border-b-0 lg:border-r">
            ...
            <Reveal delay={100}>
              <article id="for-business" className="pl-audience-panel">
                ...
                <Link to="/request-access" className="pl-btn-primary mt-8 inline-flex">
        ...
        <div className="mx-auto max-w-6xl">
          <div className="pl-validation">
            <div>
              ...
              <p className="mt-5 max-w-xl leading-8 text-[#6e6d67]">
              ...
              <div className="mt-10 space-y-5">
                {[
                  ["Predictive accuracy", "90%", "bg-[#c97f62]"],
                  ["Speed vs traditional research", "96%", "bg-[#5f8466]"],
                  ["Cost reduction", "80%", "bg-[#5f8466]"],
                  ...
                  <div key={label} className="grid grid-cols-[170px_1fr_44px] items-center gap-4 text-xs md:text-sm">
                    ...
                    <span className="font-display font-bold text-[#1f1e1c]">{width}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section id="how" className="bg-[#f4f4f2] px-5 py-24">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-12 grid overflow-hidden rounded-2xl border border-on-surface/[0.08] bg-on-surface/[0.04] sm:grid-cols-2 lg:grid-cols-4">
            {STEPS.map(({ icon: Icon, num, title, body }, i) => (
              <Reveal key={title} delay={i * 80}>
                <article className="pl-step h-full">
                  <div className="mb-5 font-mono text-xs tracking-[0.14em] text-[#c97f62]/80">{num}</div>
                  <div className="mb-5 grid h-11 w-11 place-items-center rounded-xl border border-[#c97f62]/25 bg-[#c97f62]/10 text-[#1f1e1c]">
                    <Icon size={20} />
                  </div>
                  <h3 className="font-display font-semibold">{title}</h3>
                  <p className="mt-3 text-sm leading-7 text-[#6e6d67]">{body}</p>
                </article>
              </Reveal>
            ))}
          </div>
      ...
      <section className="px-5 py-24">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-12 grid overflow-hidden rounded-3xl border border-on-surface/[0.08] bg-on-surface/[0.04] md:grid-cols-2">
            {PROVIDES.map(({ icon: Icon, title, body }, i) => (
              <Reveal key={title} delay={i * 80}>
                <article className="pl-provide-card h-full">
                  <div className="mb-5 flex items-center gap-3">
                    <span className="grid h-10 w-10 place-items-center rounded-xl border border-[#c97f62]/25 bg-[#c97f62]/10">
                      <Icon size={18} />
                    </span>
                    <h3 className="font-display font-semibold">{title}</h3>
                  </div>
                  <p className="text-sm leading-7 text-[#6e6d67]">{body}</p>
                </article>
              </Reveal>
            ))}
          </div>
      ...
      <section id="usecases" className="bg-[#f4f4f2] px-5 py-24">
        <div className="mx-auto max-w-6xl">
          <Label>Use Cases</Label>
          <h2 className="font-display text-3xl font-bold tracking-tight md:text-5xl">
            Built for how people actually use AI today.
          </h2>
          <p className="mt-4 max-w-2xl leading-8 text-[#6e6d67]">
            ...
            {USE_CASES.map(({ icon: Icon, title, body, tag, tone }, i) => (
              <Reveal key={title} delay={i * 55}>
                <article className="pl-usecase-card h-full">
                  <div className={`mb-5 grid h-10 w-10 place-items-center rounded-xl ${tone === "biz" ? "bg-[#5f8466]/10 text-[#5f8466]" : tone === "both" ? "bg-[#5f8466]/10 text-[#5f8466]" : tone === "risk" ? "bg-[#b0573f]/10 text-[#b0573f]" : "bg-[#c97f62]/10 text-[#c97f62]"}`}>
                    <Icon size={18} />
                  </div>
                  <h3 className="font-display font-semibold">{title}</h3>
                  <p className="mt-3 text-sm leading-7 text-[#6e6d67]">{body}</p>
                  <span className={`mt-5 inline-flex rounded px-2.5 py-1 font-mono text-[11px] uppercase tracking-[0.08em] ${tone === "biz" ? "bg-[#5f8466]/10 text-[#5f8466]" : tone === "both" ? "bg-[#5f8466]/10 text-[#5f8466]" : tone === "risk" ? "bg-[#b0573f]/10 text-[#b0573f]" : "bg-[#c97f62]/15 text-[#c97f62]"}`}>
                    {tag}
                  </span>
                </article>
              </Reveal>
      ...
      <section id="download" className="relative overflow-hidden px-5 py-28 text-center">
        <div className="absolute left-1/2 top-1/2 h-72 w-[640px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#c97f62]/10 blur-3xl" aria-hidden />
        <div className="relative z-10">
          <Label center>Get Started</Label>
          <h2 className="font-display text-4xl font-bold leading-tight tracking-tight md:text-6xl">
            Your persona.
            <br />
            <span className="text-[#c97f62]">Your rules.</span>
          </h2>
          <p className="mx-auto mt-5 max-w-2xl leading-8 text-[#6e6d67]">
      ...
      <footer className="flex flex-wrap items-center justify-between gap-4 border-t border-on-surface/[0.08] px-5 py-10 text-sm text-[#6e6d67] md:px-12">
