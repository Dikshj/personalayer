...
final class GRDBDatabase {
    ...
    func insertRawEvent(type: String, payload: [String: Any], connector: String? = nil) throws {
        ...
        var event = RawEvent(id: nil, eventType: type, payload: payloadStr, createdAt: Date(), privacyFiltered: false, connectorType: connector)
        try dbPool.write { db in try event.insert(db) }
    }

    func recentRawEvents(limit: Int = 100) throws -> [RawEvent] {
        try dbPool.read { db in try RawEvent.limit(limit).order(Column("createdAt").desc).fetchAll(db) }
    }
    ...
    func markPrivacyFiltered() throws {
        try dbPool.write { db in
            try db.execute(sql: """
                ...
                AND (payload LIKE '%password%' OR payload LIKE '%ssn%' OR payload LIKE '%credit_card%' OR payload LIKE '%secret%' OR payload LIKE '%token%')
            """)
        }
    }
    ...
    func deleteRawEventsOlderThan(days: Int) throws {
        ...
        try dbPool.write { db in try RawEvent.filter(Column("createdAt") < cutoff).deleteAll(db) }
    }
    ...
    func upsertNode(entityId: String, type: String, label: String, attributes: [String: Any], embedding: [Float]? = nil) throws {
        ...
        let embeddingData = embedding.flatMap {
            var data = Data()
            for f in $0 { withUnsafeBytes(of: f) { data.append(contentsOf: $0) } }
            return data
        }
        try dbPool.write { db in
            if var existing = try KGNode.filter(Column("entityId") == entityId).fetchOne(db) {
                existing.label = label; existing.attributes = attrStr; existing.embedding = embeddingData
                existing.signalStrength = min(existing.signalStrength + 0.1, 1.0)
                existing.lastAccessedAt = Date(); existing.updatedAt = Date()
                try existing.update(db)
            } else {
                var node = KGNode(id: nil, entityId: entityId, entityType: type, label: label, attributes: attrStr, embedding: embeddingData, tier: "HOT", signalStrength: 1.0, lastAccessedAt: Date(), createdAt: Date(), updatedAt: Date())
                try node.insert(db)
            }
        }
    }
    ...
    func nodesByTier(_ tier: MemoryTier, limit: Int = 100) throws -> [KGNode] {
        try dbPool.read { db in try KGNode.filter(Column("tier") == tier.rawValue).limit(limit).fetchAll(db) }
    }

    func updateNodeTier(entityId: String, tier: MemoryTier) throws {
        try dbPool.write { db in
            try db.execute(sql: "UPDATE kg_node SET tier = ?, updatedAt = ? WHERE entityId = ?", arguments: [tier.rawValue, Date(), entityId])
        }
    }

    func insertEdge(source: String, target: String, relation: String, weight: Double = 1.0, evidence: [String] = []) throws {
        let evidenceStr = String(data: try JSONSerialization.data(withJSONObject: evidence), encoding: .utf8)!
        var edge = KGEdge(id: nil, sourceEntityId: source, targetEntityId: target, relationType: relation, weight: weight, evidence: evidenceStr, createdAt: Date())
        try dbPool.write { db in try edge.insert(db) }
    }
    ...
    func insertTemporalChain(type: String, sequence: [(entityId: String, timestamp: Date)]) throws {
        ...
        var chain = TemporalChain(id: nil, chainType: type, sequence: seqStr, startDate: sequence.first?.timestamp ?? Date(), endDate: sequence.last?.timestamp ?? Date(), createdAt: Date())
        try dbPool.write { db in try chain.insert(db) }
    }
    ...
    func cleanupTemporalChains(olderThanDays: Int) throws {
        ...
        try dbPool.write { db in try TemporalChain.filter(Column("createdAt") < cutoff).deleteAll(db) }
    }
    ...
    func saveSharedBundle(_ bundle: [String: Any], userId: String = "default") throws {
        ...
        try dbPool.write { db in
            if var existing = try SharedBundle.filter(Column("userId") == userId).fetchOne(db) {
                existing.bundleJson = json; existing.updatedAt = Date()
                try existing.update(db)
            } else {
